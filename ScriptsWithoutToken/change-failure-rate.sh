...
then
    ...
    #issuesFoundInPage=$(curl -sL $GIT_REPO"/issues?state=all&since=2010-12-07T09:05:28Z&per_page=100&page=9" | grep created_at);
    flag=1
    i=1
    while [ $flag -eq 1 ]
    do
        parcialPageIssues=$(curl -sL $GIT_REPO"/issues?state=all&since=$STARTING_DATE-01T00:00:00Z&per_page=100&page=$i" | grep '"created_at":' | wc -l)
    ...
    while [ $flag -eq 1 ]
    do
        parcialPageIssues=$(curl -sL $GIT_REPO"/issues?state=all;labels=$LABEL&since=$STARTING_DATE-01T00:00:00Z&per_page=100&page=$i" | grep '"created_at":' | wc -l)
        if [ $parcialPageIssues -lt 100 ]; then
            flag=0
        fi
        i=$(($i+1));
