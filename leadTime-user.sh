...
while getopts d:m:u:t:r:o:h flag
do
    case "${flag}" in
...
then
    ...
    results=$(curl -sL $GIT_REPO/tags -u $GIT_USERNAME:$GIT_TOKEN | jq -r ".[].commit.sha");
    tagsversion=$(curl -sL $GIT_REPO/tags -u $GIT_USERNAME:$GIT_TOKEN | jq -r ".[].name");
    ...
    for i in $results; do 
        ...
        string=$(curl -sL $GIT_REPO"/commits?sha="$i"&page=1&per_page=1" -u $GIT_USERNAME:$GIT_TOKEN | jq -r ".[].commit.author.date");
        tagDates+=("$string");
    ...
    commitsAhead=$(curl -sL $GIT_REPO"/compare/$tagBeforeName...$chosenTagName" -u $GIT_USERNAME:$GIT_TOKEN | jq -r .ahead_by);
